...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('personas_informaciones_financieras', function (Blueprint $table) {
            ...
            $table->foreignId('entidad_bancaria_id')->constrained('entidades_bancarias');
            $table->boolean('tiene_prestamo');
            $table->double('monto_prestamo',10,2)->nullable();
            $table->double('cuota_mensual_prestamo',10,2)->nullable();
            $table->boolean('otro_ingreso');
            $table->double('monto_ingreso',10,2)->nullable();
            $table->string('origen_ingreso',100)->nullable();
            $table->boolean('tiene_propiedades');
            $table->string('detalle_propiedades',100)->nullable();
            $table->boolean('reportado_centrar_riesgos');
            $table->integer('entidad_reportado')->nullable(); //almacena el id de entidades financieras para futuras consultas
